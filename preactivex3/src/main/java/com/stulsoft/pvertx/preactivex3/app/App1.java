...
public class App1 extends AbstractVerticle {
    ...
    public void start() throws Exception {
        ...
        d1.subscribe(s -> {
            ...
            vertx.eventBus().rxRequest(V1.EB_ADDRESS, "")
                    .subscribe(response -> logger.info("d1.result=[{}]", response.body().toString()));
        });
        d2.subscribe(s -> {
            logger.info("d2 success = [{}]", s);
            vertx.eventBus().rxRequest(V2.EB_ADDRESS, "")
                    .subscribe(response -> logger.info("d2.result=[{}]", response.body().toString()));
        });
        super.start();
    }
}
