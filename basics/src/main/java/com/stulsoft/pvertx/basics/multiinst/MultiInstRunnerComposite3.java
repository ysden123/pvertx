 ...
 * @author Yuriy Stul
 */
public class MultiInstRunnerComposite3 {
    private static final Logger logger = LoggerFactory.getLogger(MultiInstRunnerComposite3.class);
    ...
    public static void main(String[] args) {
        ...
        vertx.deployVerticle(ServiceForMultiInst.class.getName(), deploymentOptions, deployAr -> {
            if (deployAr.succeeded()) {
                ...
                try {
                    send(vertx)
                            .concatWith(request(vertx))
                            .concatWith(publish(vertx))
                            .subscribe(() -> {
                                vertx.close();
                                logger.info("<==main");
                            });
    ...
    private static Completable request(final Vertx vertx) {
        ...
        return Completable.create(source -> {
            vertx.eventBus()
                    .request(ServiceForMultiInst.EB_ADDRESS, "do it request 1",
                            ar -> logger.info("request result: {}", ar.result().body()))
                    .request(ServiceForMultiInst.EB_ADDRESS, "do it request 2",
                            ar -> logger.info("request result: {}", ar.result().body()));
            vertx.setTimer(1000, l -> source.onComplete());
        });
    }
