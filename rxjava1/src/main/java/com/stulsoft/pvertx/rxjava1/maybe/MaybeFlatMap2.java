 ...
 * @author Yuriy Stul
 */
public class MaybeFlatMap2 {
    private static Logger logger = LoggerFactory.getLogger(MaybeFlatMap2.class);
    ...
    private static Disposable handle(Maybe<Integer> mayBe) {
        return mayBe
                .flatMap(
                        i -> Maybe.just(i + 1000),    // on success mapper
                        e -> Maybe.just(123),         // on error mapper
                        () -> Maybe.just(321))        // on complete mapper
